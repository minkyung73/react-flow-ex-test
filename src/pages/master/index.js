  ...
  const onConnect = useCallback(
    (params) => {
      ...
      if (
        sourceNode &&
        targetNode &&
        Math.abs(sourceNode.depth - targetNode.depth) !== 1
      ) {
      ...
      const isAlreadyConnectedSource =
        sourceNode.depth === 3 &&
      ...
      const isAlreadyConnectedTarget =
        targetNode.depth === 4 &&
    ...
    (event, connectionState) => {
      if (!connectionState.isValid) {
        ...
        const isAlreadyConnected =
          fromNodeDepth === 3 &&
          edges.some((edge) => edge.source === fromNodeId);
        if (isAlreadyConnected) {
          console.log(
            "Cannot create a new node: fromNode is already connected."
          );
          return;
        }
    ...
    >
      <div style={{ flex: 1 }}>
        ...
        >
          ...
          {/* <div>
            <button onClick={onCheckNodes}>check the nodes</button>
            <button onClick={onCheckEdges}>check the edges</button>
          </div> */}
          {/* </Panel> */}
        </ReactFlow>
      </div>
      <div
        className="update-node__controls"
        style={{ width: "300px", padding: "10px" }}
      >
        <label>data(label): </label>
        <input
          value={curNode && curNode.data ? curNode.data.label : ""}
